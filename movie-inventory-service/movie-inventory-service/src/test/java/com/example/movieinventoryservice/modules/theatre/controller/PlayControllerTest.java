...
//
//		MvcResult result = this.mockMvc.perform(get("/play/id/1")).andExpect(MockMvcResultMatchers.status().isOk())
//				.andReturn();
...
//		MvcResult result = mockMvc.perform(post("/play/add").contentType(MediaType.APPLICATION_JSON)
...
//		MvcResult result = mockMvc.perform(put("/play/update").contentType(MediaType.APPLICATION_JSON)
//				.content(getPlaysAsJson()).characterEncoding("utf-8")).andExpect(status().isOk()).andReturn();
//		System.out.println(result.getResponse().getContentAsString());
...
//		MvcResult result = this.mockMvc.perform(delete("/play/id/1"))
